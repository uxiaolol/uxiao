...
--[[			local file,l,f,jiy,peny,ourline
		...
		elseif DmFindPic('start.bmp',90,801,464,810,475) then
			click(x,y)  
		--账号封停
		elseif DmFindPic('fenghao.bmp',85,514,177,534,194) and DmFindPic('dlyc.bmp',90,542,490,553,502) and isok[i] == 1 then
			click(x,y)
		...
		elseif DmFindPic('denglu1.bmp',90,841,108,854,121) then
			...
			while os.difftime(os.time(),tt) < 4 do
				if DmFindPic('zhbcz1.bmp',80,216,420,232,435,i) then					
					local file,l,f,jiy,peny,ourline
					file = io.open("/var/touchelf/scripts/avf"..i..".txt","r")
					ourline = tonumber(file:read())		
					delete(ourline,i)			
...
function levelup()
	...
	while (i<3) do
		...
		elseif (DmFindPicFuzzy('3zhuan80.bmp',90,82,232,131,249,0xffffff) or DmFindPicFuzzy('2zhuan60.bmp',90,.82,233,130,248,0xffffff) or DmFindPicFuzzy('2zhuan50.bmp',90,81,233,131,249,0xffffff)) and DmFindPic('bag.bmp',90,831,615,842,628) then
			...
			isfinish[i] = 1
		--3转60
		elseif DmFindPicFuzzy('3zhuan.bmp',90,81,232,110,251,0xffffff) and DmFindPic('bag.bmp',90,831,615,842,628) then
			notifyMessage('到达一转a') mSleep(2000) 
			logDebug('到达一转1')	
			isfinish[i] = 1
...
function using(i)
	...
	while true do		
		...
		elseif DmFindPic('cz.bmp',85,460,259,471,269) or DmFindPic('cz1.bmp',85,460,222,472,232) then--795,105
			...
			break
		--彩色羽毛
		elseif DmFindPic('caise.bmp',85,633,166,652,180) or DmFindPic('caise1.bmp',85,627,119,650,135) then --798,142
			click(x+165,y-24)
			closewin()
			break
		--玛雅晶石
...
--[[			appKill(packagename[i])
			...
			logDebug('kill'..':'..packagename[i])	--]]
			if DmFindPic('map_gu.bmp',85,1012,18,1026,32) then--587,299
				click(x,y+160) 		
			else
				appKill(packagename[i])
				ft[i]=os.time()
...
--[[	local i = 1--]]
	...
	while true do
		...
		elseif DmFindPic('liaotian.bmp',90,757,591,778,601) then
			click(x,y)
		elseif isfinish[1]==1 or isfinish[2]==1 then
			break
		elseif  DmFindPic('deal.bmp',90,432,572,446,584) == false and isdeal[i]>=2 then
			isfinish[i]=1
		--密语	
